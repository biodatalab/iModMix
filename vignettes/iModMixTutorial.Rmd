...
Since this is purely an example of how to run the entire analysis from start to finish, we are going to limit our analysis to 1000 first features.
...
## Enrichment analysis for gene Symbol 

Enrichment analysis can be perform is annotation data for proteomics or transcriptomics is uploaded, and the column Symbol is available.  Drop-down menu displays available libraries for pathway analysis. Choose a library to automatically amend the dataset cluster descriptions on table below. For the example transcrptomics Data2 we selec "GO_Biological_Process_2023". Under column enriched_Term the most highly correlated pathway is displayed and in the following columns, along with enriched_Genes, and p-values as determined by Enrichr.
